# clean-jsdoc-theme — JSDoc example

A standalone example showing how to set up [clean-jsdoc-theme](https://github.com/ankitskvmdam/clean-jsdoc-theme) **v5** with JSDoc.

This is a working reference — clone it, install, run the generate script, and you'll get a fully rendered documentation site under `docs/`. Use its `jsdoc.json`, `package.json`, and `src/` as a template for your own project.
...
```json
{
  ...
  "templates": {
    "default": {
      // Standard JSDoc option — emit linkable source-code pages.
  ...
  "opts": {
    ...
    // clean-jsdoc-theme options go here, alongside the standard opts.
    "siteName": {
      "light": "./assets/clean-jsdoc-theme.svg",
      "dark": "./assets/clean-jsdoc-theme-white.svg",
      "alt": "JSDoc example"
    }
  }
}
...
- **clean-jsdoc-theme options live under `opts`** (alongside `template`, `destination`, etc.) — not under `opts.theme_opts`, which was the v4 location. The one exception below, `outputSourceFiles`, is a standard JSDoc template option and stays under `templates.default`.

### Theme options

Common options you can set under `opts`:
...
| `outputSourceFiles` | Emit linkable highlighted source-code pages. *(standard JSDoc option — set under `templates.default`, not `opts`.)* |
